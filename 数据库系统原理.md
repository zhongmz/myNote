...
- $$
  _{A_1,A_2}G_{sum(A_3)}(\Pi _{A_1,A_2...A_n}(\sigma_P(r_1*r_2..r_n)))
  $$
...
- R=(A,B,C)
  S = (D, E, F)
  关系r(R)和s(S)已给出。请给出与下列每个表达式等价的元组关系演算表达式：
...
- 这个例子是不可调度的，因为$T_7$的commit在$T_6$结束之前就提交了

### 什么是级联回滚？

- 因为单个事务故障导致1了一系列事务回滚的现象称为级联回滚。

### 什么是无级联调度？

- 无级联回滚应该满足：对于每对事务$T_i$和$T_j$，如果$T_j$读取了先前由$T_i$所写的数据，则$T_i$必须在$T_j$**这一读操作**前提交。
...
- 若一个数据项已经被另外一个尚未提交或者中止的事务写入，则不允许对该数据执行写操作
...
- 将两阶段的封锁修改为严格两阶段协议【要求事务锁有排他锁必须**在事务提交[commit]后**才释放】
...
- 因为事务访问多个数据项的时候，则需要加多个锁，而这些枷锁操作是很**费时**的
...
- 搜索整个树【但是破坏了多粒度机制的初衷】
- 引入**意向型锁**：在一个结点显式加锁之前， 该结点的全部祖先结点均加上了对应的意向锁
